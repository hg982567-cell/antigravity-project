...
async function main() {
  console.log("🌱 Seeding DropAI Database...");
  ...
  await prisma.securityEvent.create({
    data: {
      ...
      metadata: JSON.stringify({ method: "password", twoFactorRequired: false }),
    },
  });
  ...
  await prisma.auditLog.create({
    data: {
      ...
      entityType: "STORE",
      entityId: store1.id,
      newValue: JSON.stringify({ storeName: store1.name, platform: "SHOPIFY" }),
      ipAddress: "192.168.1.105",
    },
  });
